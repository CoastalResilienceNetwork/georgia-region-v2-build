...
return isClamped ? floor(antiAliasedResult + 0.5) : antiAliasedResult;
  ...
  `)}function Sr(t){return t?(Math.floor(.5*(fr(t)-1))+.5)/t.pixelRatio:1}function wt(t){const e=t.stipplePattern;return e?hr(t.stipplePattern)/e.pixelRatio:1}const _r=.4,Nt=64,Tr=Nt/2,br=Tr/5,yr=Nt/br,Oa=.25;function Or(t,e){const i=t.vertex;Ye(i),i.uniforms.get("markerScale")==null&&i.constants.add("markerScale","float",1),i.constants.add("markerSizePerLineWidth","float",yr).code.add(p`float getLineWidth() {
...
prev = projectAndScale(prev);
  ...
  `),y&&a.main.add(p`vec4 other = isStartVertex ? next : prev;
...
vec2(vStippleDistanceLimits.x, 1e34);`)),a.main.add(p`
    ...
    }`),e.fragment.include(bi,t),e.include(yi,t),s.include(Oi),s.main.add(p`discardBySlice(vpos);
