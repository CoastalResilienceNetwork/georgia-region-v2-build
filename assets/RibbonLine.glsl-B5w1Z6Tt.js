...
}`))}function $t(t){return t.pattern.map(e=>Math.round(e*t.pixelRatio))}function hr(t){if(t==null)return 1;const e=$t(t);return Math.floor(e.reduce((i,r)=>i+r))}function fr(t){return $t(t).reduce((e,i)=>Math.max(e,i))}function mr(t){return t==null?Yt:t.length===4?t:qt(vr,t[0],t[1],t[2],1)}const vr=Xt();function gr(t,e){if(!e.stippleEnabled)return void t.fragment.code.add(p`float getStippleAlpha() { return 1.0; }
void discardByStippleAlpha(float stippleAlpha, float threshold) {}
vec4 blendStipple(vec4 color, float stippleAlpha) { return color; }`);const i=!(e.draped&&e.stipplePreferContinuous),{vertex:r,fragment:a}=t;a.include(qi),e.draped||(ui(r,e),r.uniforms.add(new Ye("worldToScreenPerDistanceRatio",({camera:s})=>1/s.perScreenPixelRatio)).code.add(p`float computeWorldToScreenRatio(vec3 segmentCenter) {
...
}`),a.uniforms.add(new hi("stipplePatternTexture",s=>s.stippleTexture),new Y("stipplePatternSDFNormalizer",s=>Sr(s.stipplePattern)),new Y("stipplePatternPixelSizeInv",s=>1/wt(s))),e.stippleOffColorEnabled&&a.uniforms.add(new Be("stippleOffColor",s=>mr(s.stippleOffColor))),a.code.add(p`float getStippleSDF(out bool isClamped) {
...
return isClamped ? floor(antiAliasedResult + 0.5) : antiAliasedResult;
  ...
  `)}function Sr(t){return t?(Math.floor(.5*(fr(t)-1))+.5)/t.pixelRatio:1}function wt(t){const e=t.stipplePattern;return e?hr(t.stipplePattern)/e.pixelRatio:1}const Tr=.4,Nt=64,_r=Nt/2,br=_r/5,Or=Nt/br,ya=.25;function yr(t,e){const i=t.vertex;qe(i),i.uniforms.get("markerScale")==null&&i.constants.add("markerScale","float",1),i.constants.add("markerSizePerLineWidth","float",Or).code.add(p`float getLineWidth() {
...
float segmentLengthPseudoScreen = segmentLengthRender * discreteWorldToScreenRatio;`),a.uniforms.add(new Y("stipplePatternPixelSize",v=>wt(v))),a.main.add(p`float patternLength = lineSize * stipplePatternPixelSize;
